...
namespace YAML {
	...
	template<>
	struct convert<Acrylic::UUID>
	{
		static Node encode(const Acrylic::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Acrylic::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	{
		AC_CORE_ASSERT(entity.HasComponent<IDComponent>());
				...
				if (scriptComponent)
				{
					...
					if (scriptFields)
					{
						...
						for (auto scriptField : scriptFields)
						{
							...
							// TODO: turn this assert into Acrylic log warning
							AC_CORE_ASSERT(fields.find(name) != fields.end());
