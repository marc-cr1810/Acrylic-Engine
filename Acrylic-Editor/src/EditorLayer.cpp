...
namespace Acrylic 
{
						...
						if (m_HoveredEntity)
						{
							if (m_HoveredEntity.HasComponent<SpriteRendererComponent>())
							{
								...
								if (texture->IsLoaded())
									...
									AC_WARN("Could not load texture {0}", file.filename().string());
							}
						}
					}
				}
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		if (selectedEnitity && m_SceneState != SceneState::Play)
		{
	...
	{
		AC_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
